...
fn test_complete_indexing_and_search_workflow() {
    ...
    let ai_ml_results =
        index.boolean_search(BooleanOperator::And, vec!["artificial", "intelligence"]);
    ...
    for result in &wildcard_results {
        ...
        assert!(
            full_text.contains("computational")
...
fn test_tfidf_ranking_accuracy() {
    ...
    index.add_document(
        ...
        "Machine learning uses machine algorithms to learn patterns from data automatically."
            .to_string(),
    );
    ...
    assert!(highest_score_doc.title == "Machine Learning" || results[0].score > 0.0); // At minimum, ensure scores are positive
}
...
fn test_phrase_search_precision() {
    ...
    index.add_document(
        "AI and ML".to_string(),
        "Artificial intelligence and machine learning are related but different fields."
            .to_string(),
    );
...
fn test_boolean_search_logic() {
    ...
    index.add_document(
        ...
        "artificial neural networks".to_string(),
    );

    // Doc 3: Has B only
    index.add_document(
        "Document B".to_string(),
        "machine intelligence algorithms".to_string(),
    );
    ...
    let not_results =
        index.boolean_search(BooleanOperator::Not, vec!["artificial", "intelligence"]);
    assert_eq!(not_results.len(), 1); // Should find only doc 2

    let not_doc = index.get_document(not_results[0].doc_id).unwrap();
    let full_text = format!(
        "{} {}",
        not_doc.title.to_lowercase(),
        not_doc.content.to_lowercase()
    );
    assert!(full_text.contains("artificial") && !full_text.contains("intelligence"));
}
...
fn test_wildcard_pattern_matching() {
    ...
    index.add_document(
        ...
        "Programming languages include Python, JavaScript, and Rust for different applications."
            .to_string(),
    );
    ...
    for result in &suffix_results {
        ...
        assert!(
            full_text.contains("programming")
...
fn test_large_document_performance() {
    ...
    for i in 0..1000 {
        large_content.push_str(&format!(
            "This is sentence {} about machine learning and artificial intelligence. ",
            i
        ));
    }
...
fn test_snippet_generation_quality() {
    ...
    assert!(
        snippet.to_lowercase().contains("learning")
